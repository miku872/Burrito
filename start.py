...
import Constants
import datetime
from Utils import CalanderUtil
...
def getComparator():
    ...
    day = date.weekday()
    if day == 5 or day == 6:
        print("This is weekend, returning last trading working day")
    else:
        print("The timestamp is not on a Saturday.")

    return str(datetime.date.today())


def getSeries(apiProvider, symbol, candleSize, start=None, end=datetime.date.today()):
    try:
        if start is None:
