...
class ReverseHammer(Scan):
    ...
    def isCriteriaMet(self, symbol, candleSize="DAILY", apiProvider=None, timeSeries=None, interval=None):
        ...
        if timeSeries is not None:
            ...
            if ((close0 > open0 > low0 and (high0 - close0) >= 3 * (close0 - open0) and (open0 - low0) < (
                    close0 - open0)) or
                    (open0 > close0 > low0 and (high0 - open0) >= 3 * (open0 - close0) and (close0 - low0) < (
                            open0 - close0))):
                return True
        return False
